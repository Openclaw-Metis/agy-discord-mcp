...
describe('attachmentRootCandidates', () => {
  ...
  const imageDir = join(sep, 'home', 'imgs')

  it('uses cwd, AGY_WORKDIR, inbox, and the image dir by default', () => {
    const roots = attachmentRootCandidates(inbox, {
      AGY_WORKDIR: join(sep, 'work'),
      AGY_DISCORD_GENERATED_IMAGES_DIR: imageDir,
    })
  ...
  it('replaces cwd/workdir with configured roots but always keeps inbox + image dir', () => {
    ...
    const roots = attachmentRootCandidates(inbox, {
      AGY_DISCORD_ATTACHMENT_ROOTS: exportsDir,
      AGY_WORKDIR: join(sep, 'work'),
      AGY_DISCORD_GENERATED_IMAGES_DIR: imageDir,
    })
