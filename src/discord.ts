...
export class DiscordBridge extends EventEmitter {
  ...
  private async handleInbound(message: Message): Promise<void> {
    ...
    if (gate.action === 'pair') {
      ...
      await message
        .reply(
          `${lead}. Run this on the agy host:\n\nagy-discord-mcp access pair ${gate.code}`,
        )
        .catch(err => {
  ...
  private checkApprovals(): void {
    ...
    for (const senderId of files) {
      ...
      this.fetchTextChannel(chatId)
        .then(channel => {
          if ('send' in channel) return channel.send('Paired. Send a message to agy.')
          return undefined
        })
        .then(() => {
  ...
  private assertSendable(file: string): void {
    ...
    if (!roots.some(root => isPathInsideOrEqual(realFile, root))) {
      throw new Error(
        `refusing to send file outside allowed attachment roots: ${file}. Set AGY_DISCORD_ATTACHMENT_ROOTS to opt in.`,
      )
    }
  }
...
// Candidate attachment roots before symlink resolution. agy's image
// output directory is always appended so generated images stay attachable
// regardless of AGY_DISCORD_ATTACHMENT_ROOTS.
export function attachmentRootCandidates(
  inboxDir: string,
  env: NodeJS.ProcessEnv = process.env,
): string[] {
  const configured = env.AGY_DISCORD_ATTACHMENT_ROOTS
  const base = configured?.trim()
    ? [...configured.split(delimiter), inboxDir]
    : [process.cwd(), env.AGY_WORKDIR, inboxDir]
...
// This is the boundary that stops agy from sending to non-allowlisted channels.
