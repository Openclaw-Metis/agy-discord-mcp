...
import { agyOptionsFromEnv } from './agy.js'
import {
  approvePendingCode,
  defaultAccess,
  ensureStateDir,
...
async function initCommand(): Promise<void> {
  ...
  try {
    process.stdout.write(`agy-discord-mcp init\nstate directory: ${paths.stateDir}\n\n`)
...
async function tokenFromConfigureArgs(args: string[]): Promise<string> {
  ...
  if (tokenEnvIndex >= 0) {
    ...
    if (!envName) {
      throw new Error('Usage: agy-discord-mcp configure --token-env <env-var-name>')
    }
...
async function accessCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'pair': {
      const code = rest[0]
      if (!code) throw new Error('Usage: agy-discord-mcp access pair <code>')
    ...
    case 'policy': {
      ...
      if (policy !== 'pairing' && policy !== 'allowlist' && policy !== 'disabled') {
        throw new Error('Usage: agy-discord-mcp access policy <pairing|allowlist|disabled>')
      }
    ...
    case 'allow-user': {
      const userId = rest[0]
      if (!userId) throw new Error('Usage: agy-discord-mcp access allow-user <discord-user-id>')
    ...
    case 'remove-user': {
      const userId = rest[0]
      if (!userId) throw new Error('Usage: agy-discord-mcp access remove-user <discord-user-id>')
    ...
    case 'allow-channel': {
      ...
      if (!channelId) {
        throw new Error(
          'Usage: agy-discord-mcp access allow-channel <channel-id> [--no-mention] [--allow-user <user-id>...]',
        )
      }
    ...
    case 'remove-channel': {
      const channelId = rest[0]
      if (!channelId) {
        throw new Error('Usage: agy-discord-mcp access remove-channel <channel-id>')
      }
    ...
    case 'ack': {
      const value = rest[0]
      if (!value) throw new Error('Usage: agy-discord-mcp access ack <emoji|off>')
...
async function mentionPatternCommand(args: string[]): Promise<void> {
  ...
  if (action === 'add') {
    if (!pattern) throw new Error('Usage: agy-discord-mcp access mention-pattern add <regex>')
  ...
  if (action === 'remove') {
    if (!pattern) throw new Error('Usage: agy-discord-mcp access mention-pattern remove <regex>')
  ...
  throw new Error('Usage: agy-discord-mcp access mention-pattern <list|add|remove>')
}

function inviteUrlCommand(args: string[]): void {
  const clientId = args[0]
  if (!clientId) throw new Error('Usage: agy-discord-mcp invite-url <discord-client-id>')
...
export function buildConfigSnippet(options: { useNpx?: boolean; cliPath?: string } = {}): string {
  ...
  const commandArgs = useNpx ? ['-y', 'agy-discord-mcp', 'mcp'] : [cliPath, 'mcp']
  ...
  return `# Merge this into ~/.gemini/settings.json (or ~/.gemini/antigravity-cli/settings.json):

${JSON.stringify(snippet, null, 2)}

# Token can live in ${getStatePaths().envFile}, or add an "env" block to the
# discord server above, e.g. "env": { "DISCORD_BOT_TOKEN": "..." }.
`
}
...
export function collectDoctorReport(): DoctorReport {
  ...
  const checks: Record<string, DoctorCheck> = {
    ...
    agy: checkAgyCommand(agyOptions.command),
    stateDir: {
    ...
    workdir: {
      ok: exists(agyOptions.workdir),
      path: agyOptions.workdir,
      message: exists(agyOptions.workdir) ? 'present' : 'missing',
    },
    sandbox: {
      ok: true,
      value: agyOptions.sandbox ? 'on' : 'off',
    },
  }

  return {
    ok: checks.node.ok && checks.agy.ok && checks.discordToken.ok && checks.workdir.ok,
    checks,
  }
}
...
function printHelp(): void {
  process.stdout.write(`agy-discord-mcp

Commands:
  mcp                       Run the Discord MCP server over stdio
  bot                       Run Discord -> agy --print relay mode
  ...
  print-config [--npx]       Print an agy mcpServers JSON config snippet
  doctor [--json]            Show basic local configuration status

Run "agy-discord-mcp access help" for access commands.
`)
}

function printAccessHelp(): void {
  process.stdout.write(`agy-discord-mcp access commands:
...
function checkAgyCommand(command: string): DoctorCheck {
